...
case $opt in
        ...
        deploy-worker)
            ...
            if [ ${env} == "staging" ]; then
                queue_name=$(curl -k -L -X GET -H "Authorization: Token $token" https://staging.mukayese.tdd.ai/api/challenges/get_broker_url/$challenge/)
            elif [ ${env} == "production" ]; then
                queue_name=$(curl -k -L -X GET -H "Authorization: Token $token" https://mukayese.tdd.ai/api/challenges/get_broker_url/$challenge/)
            fi
        ...
        deploy-workers)
            ...
            if [ ${env} == "staging" ]; then
                queue_names=$(curl -k -L -X GET -H "Authorization: Token $token" https://staging.mukayese.tdd.ai/api/challenges/get_broker_urls/)
            elif [ ${env} == "production" ]; then
                queue_names=$(curl -k -L -X GET -H "Authorization: Token $token" https://mukayese.tdd.ai/api/challenges/get_broker_urls/)
            fi
